# Native health bridge — reference implementation

This is the Apple HealthKit / Health Connect implementation that shipped in
`mobile/src/native/health.ts` under Expo SDK 52, preserved verbatim.

It was removed from the live module when the project moved to SDK 57, because
both libraries it depends on are old-architecture bridge modules and the legacy
architecture was removed in SDK 55 / React Native 0.82. The libraries cannot
load at all on the current runtime.

The valuable part is not the plumbing — it is the exact permission strings and
record types, which are tedious to rediscover and easy to get subtly wrong.
Those are kept as live constants in `health.ts`; the rest is here.

When reimplementing against new-architecture libraries, the server contract does
not change: `POST` a list of `DayPayload` objects via `api.fitness.pushHealth`,
one per calendar day, keyed on (user, day, provider), upserted server-side.
